...
const LEASH_ADDRESS = required("NEXT_PUBLIC_LEASH_MANAGER_ADDRESS") as `0x${string}`;
const PLEDGE_ADDRESS = required("NEXT_PUBLIC_PLEDGE_VAULT_ADDRESS") as `0x${string}`;
 ...
 */
async function backfill(): Promise<bigint> {
  ...
  const tick = async () => {
    try {
      ...
      if (head > cursor) {
        ...
        if (count > 0) log(`applied ${count} event(s) through block ${head}`);
        cursor = head;
        await writeCursor(cursor);
      }
    } catch (error) {
