 ...
 */
export async function onLeashCreated(ctx, args) {
...
export async function onLeashSpent(ctx, args) {
...
export async function onLeashRevoked(ctx, args) {
    const { data: leash } = await ctx.db
        .from("leashes")
        .select("id, owner_id, revoked")
        .eq("onchain_leash_id", args.leashId)
        .maybeSingle();
    if (!leash || leash.revoked)
        return;
...
export async function onPledgeCreated(ctx, args) {
    const { data } = await ctx.db
        .from("pledges")
...
async function resolvePledge(ctx, pledgeId, succeeded, eventType, amount) {
...
export async function onPledgeSucceeded(ctx, args) {
    await resolvePledge(ctx, args.pledgeId, true, "succeeded", args.amountReturned);
}
export async function onPledgeFailed(ctx, args) {
    await resolvePledge(ctx, args.pledgeId, false, "failed", args.amountSlashed);
}
export async function onPledgeExpiredSlashed(ctx, args) {
