...
export function listDestinations(): FailureDestination[] {
...
export function resolveDestination(
  ...
  if (!match) throw new Error(`Unknown destination: ${id}`);
  if (!match.address) {
    throw new Error(
      `${match.label} isn't configured yet. Pick another destination or use a custom address.`
    );
  }
