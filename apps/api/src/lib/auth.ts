...
let magicSdk: Magic | null = null;
async function magic(): Promise<Magic> {
  if (!magicSdk) magicSdk = await Magic.init(env.magicSecretKey);
  return magicSdk;
}
 ...
 */
export async function verifyMagicToken(didToken: string): Promise<MagicIdentity> {
 ...
 */
function authEmailForAddress(address: string): string {
...
export async function mintSession(
  address: string,
  meta: { email?: string | null; magicIssuer?: string | null }
): Promise<Session> {
  const db = serviceDb();
  ...
  await db
    .from("users")
    .upsert(
      { id: userId, address: normalized, email: meta.email ?? null, magic_id: meta.magicIssuer ?? null },
      { onConflict: "id" }
    );
