...
function accountFor(ownerAddress) {
...
export async function getUnifiedBalance(ownerAddress) {
    const assets = await accountFor(ownerAddress).getPrimaryAssets();
    const byChain = new Map();
    const tokens = new Set();
    for (const asset of assets.assets ?? []) {
        if (asset.amountInUSD > 0)
            tokens.add(String(asset.tokenType ?? "").toUpperCase());
...
export async function chainsHoldingValue(ownerAddress) {
    try {
