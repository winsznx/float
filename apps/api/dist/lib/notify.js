...
async function record(userId, type, payload) {
    if (!userId)
        return;
    await serviceDb()
        .from("notifications")
...
export async function notifyClaim(params) {
    ...
    await resend().emails.send({
        ...
        html: shell(`You've received $${params.amount.toFixed(2)}`, "Someone sent you money on FLOAT. Claim it with your email — no wallet or app install needed.", { label: "Claim it", url }),
    });
}
export async function notifySettleRequest(params) {
    ...
    await resend().emails.send({
        ...
        html: shell(`You owe $${params.share.toFixed(2)}`, `${params.splitName ?? "A split"} was shared with you. Settle from whatever you hold — FLOAT handles the rest.`, { label: "Settle up", url }),
    });
}
export async function notifyLeashGranted(params) {
    ...
    await resend().emails.send({
        ...
        html: shell(`You've been given a spending key`, `You can spend up to $${params.limit.toFixed(2)} from someone's FLOAT balance, within the limits they set. They stay in control and can revoke at any time.`, { label: "Open your key", url }),
    });
}
export async function notifyWitness(params) {
    ...
    await resend().emails.send({
        ...
        html: shell("You're the witness", `"${params.goal}" — $${params.stake.toFixed(2)} is at stake. Confirm whether they made it.`, { label: "Give your verdict", url }),
    });
}
