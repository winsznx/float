...
async function isKnownUser(address) {
 ...
 */
async function resolveFarcaster(username) {
    const handle = username.replace(/^@/, "");
    const response = await fetch(`https://api.neynar.com/v2/farcaster/user/by_username?username=${encodeURIComponent(handle)}`, { headers: { "x-api-key": env.neynarApiKey }, signal: AbortSignal.timeout(10_000) });
    if (!response.ok)
        return null;
...
export async function resolveIdentity(input) {
    ...
    if (!trimmed)
        throw new Error("Enter a name, handle, or email.");
    const base = { input: trimmed, chains: [], preferredChain: "Arbitrum" };
    const finish = async (type, address) => {
        if (!address) {
